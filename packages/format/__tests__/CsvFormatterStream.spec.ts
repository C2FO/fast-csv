...
describe('CsvFormatterStream', () => {
    ...
    const formatRows = <I extends Row, O extends Row>(rows: Row[], options: FormatterOptionsArgs<I, O> = {}) => {
        return pipeToRecordingStream(new CsvFormatterStream(new FormatterOptions(options)), rows);
    };

    it('should write an array of arrays', () => {
        return expect(formatRows(arrayRows, { headers: true })).resolves.toEqual(['a,b', '\na1,b1', '\na2,b2']);
    });

    it('should write an array of objects', () => {
        return expect(formatRows(objectRows, { headers: true })).resolves.toEqual(['a,b', '\na1,b1', '\na2,b2']);
    });
        ...
        it('should support transforming an array of multi-dimensional arrays', () => {
            return expect(
                formatRows(multiDimensionalRows, {
                    headers: true,
                    transform(row: Row) {
                        return (row as RowHashArray<string>).map((entry) => {
    ...
    describe('quoteColumns option', () => {
        describe('quote all columns and headers if quoteColumns is true and quoteHeaders is false', () => {
            ...
            it('should work with objects', () => {
                return expect(formatRows(objectRows, opts)).resolves.toEqual(['"a","b"', '\n"a1","b1"', '\n"a2","b2"']);
            });

            it('should work with arrays', () => {
                return expect(formatRows(arrayRows, opts)).resolves.toEqual(['"a","b"', '\n"a1","b1"', '\n"a2","b2"']);
            });

            it('should work with multi-dimenional arrays', () => {
                return expect(formatRows(multiDimensionalRows, opts)).resolves.toEqual([
                    '"a","b"',
                    '\n"a1","b1"',
                    '\n"a2","b2"',
                ]);
        ...
        describe('if quoteColumns is false and quoteHeaders is an object it should only quote the specified header and not the column', () => {
            ...
            it('should work with multi-dimenional arrays', () => {
                return expect(formatRows(multiDimensionalRows, opts)).resolves.toEqual(['"a",b', '\na1,b1', '\na2,b2']);
            });
        });
    ...
    describe('header option', () => {
        it('should write an array of objects without headers', () => {
            return expect(formatRows(objectRows, { headers: false })).resolves.toEqual(['a1,b1', '\na2,b2']);
        });
        ...
        it('should write an array of multi-dimensional arrays without headers', () => {
            return expect(formatRows(multiDimensionalRows, { headers: false })).resolves.toEqual(['a1,b1', '\na2,b2']);
        });

        it('should write an array of multi-dimensional arrays with headers', () => {
            return expect(formatRows(multiDimensionalRows, { headers: true })).resolves.toEqual([
                'a,b',
                '\na1,b1',
                '\na2,b2',
            ]);
