...
import {
    ParserOptions, ParserOptionsArgs, RowArray, RowValidationResult,
} from '../../../src/parser';
...
describe('HeaderTransformer', () => {
    ...
    const transform = (row: RowArray, transformer: HeaderTransformer): Promise<RowValidationResult> => new Promise((res, rej) => {
    ...
    describe('#transform', () => {
        ...
        it('should throw an error if headers is not defined and renameHeaders is true', () => {
            ...
            return transform(row1, transformer)
                .then(
                    ...
                    err => assert.strictEqual(err.message, 'Error renaming headers: new headers must be provided in an array')
                );
        });
        ...
        it('should throw an error if the row length is > than the headers length as strictColumnHandling is not defined', () => {
            const row = [ 'a', 'b', 'c' ];
            const transformer = createHeaderTransformer({ headers: [ 'header1', 'header2' ] });
            return transform(row, transformer)
                .then(
                    () => assert.fail('should have failed'),
                    err => assert.strictEqual(err.message, 'Unexpected Error: column header mismatch expected: 2 columns got: 3')
                );
        });

        it('should throw an error if the row length is > than the headers length as strictColumnHandling is false', () => {
            const row = [ 'a', 'b', 'c' ];
            const transformer = createHeaderTransformer({ headers: [ 'header1', 'header2' ], strictColumnHandling: false });
            return transform(row, transformer)
                .then(
                    () => assert.fail('should have failed'),
                    err => assert.strictEqual(err.message, 'Unexpected Error: column header mismatch expected: 2 columns got: 3')
        ...
        it('should mark the row as invalid if the row length is > than the headers length as strictColumnHandling is true', () => {
            const row = [ 'a', 'b', 'c' ];
            const transformer = createHeaderTransformer({ headers: [ 'header1', 'header2' ], strictColumnHandling: true });
            return transform(row, transformer)
                .then((results) => {
